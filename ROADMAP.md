...
| v1.5.0 | ✅ Credit grants take a custom quantity + note from both `/gift` (parsed from "Sam 3 covered setup") and the People page; recipient notifications pluralize. New "Recent credit grants" audit table on the People screen. Test upkeep: the bot's router graph is built once in `TelegramTestCase::updateRouter()`, so a new collaborator is one edit rather than one per bot test |
| v1.6.1 | ✅ Refactor: the board's "what it looks like" half (task grouping + button/keyboard building) extracted from `BoardService` into a `BoardRenderer`, so posting/editing the message reads separately from composing it. No behaviour change; `BoardService::render()` still delegates, and the renderer is built internally so no wiring changed |
| v1.6.2 | ✅ Refactor: the public signup board's composition (`viewModel` grouping + occupancy + standing/ticket bits) extracted from `SignupController` into a `SignupBoardView` that takes the resolved person, so the presentation reads apart from the request handling. `viewModel()` still delegates; built internally, no wiring changed |
| v1.9.2 | ✅ Merged Leadership into the People page as a real in-page tab (matching the Settings tabs), resolving the inconsistency of link-tabs plus separate menu entries. `PeoplePage` absorbs the leader-eligibility data; `LeadersPage` and `leaders.php` are retired; the Leadership menu item is gone (menu: Tasks · Roster · People · Settings · Diagnostics). Editing a person from the Leadership list forces the People tab |
| v1.9.1 | ✅ Fixes on the 1.9.0 admin work: restored Leadership page access (`remove_submenu_page` also strips the capability `user_can_access_admin_page` checks, so it's a normal menu item again, still grouped by the People tab); fixed a 0-capacity role generating a capacity-1 task via the second, missed clamp in `TaskTemplate::build` (`max(1→0)`), so parked roles stay off the board; Settings tab order now leads with **Roles** (renamed from "Roles & tasks") and Cancellations moved to the Reputation & alerts tab |
| v1.9.0 | ✅ Admin UX overhaul: Settings split into four tabs (Telegram bot · Web page · Roles & tasks · Reputation & alerts) via a `nav-tab-wrapper` + progressive-enhancement JS over one form/one Save (remembers the last tab), Telegram-first; a self-ticking "Getting started" checklist on Tasks (`TasksPage::setupState`); Leadership lifted out of the menu (`remove_submenu_page`) into a People sub-tab; a green/amber/red health banner on Diagnostics; advanced reputation outcome-weights folded into a `<details>` |
| v1.8.3 | ✅ The webhook + command menu re-install automatically after a plugin update: `SettingsPage::installOnUpdate` (on `admin_init`, gated by a stored `eventcrew_webhook_version`, mirrors `Schema::maybeMigrate`) reuses the extracted `installWebhook()` the button now also calls. The `/notifications` keyboard is now three columns (name no-op · 📱 Telegram · ✉ email) with a header row, matching the web tab |
| v1.8.2 | ✅ Removed the account **disable/pause** concept entirely (`disabled_at` column, `Person::isDisabled`/`acceptsOpenTaskEmail`, `PersonRepository::disable/enable`, the `ManageController` disable action, and the `isDisabled` email gates in `NotificationPreferences`/`ClaimNotifier`/`ReminderCall`/`LeaderEligibilityNotifier`). Email is governed solely by the per-type Notifications preferences now. Schema bumped to 9 with a guarded `DROP COLUMN disabled_at` migration. Fixes wanted emails (gifted credit, reminder, confirmation) being silently swallowed by a stale disabled flag |
| v1.8.1 | ✅ New `/web` command (`ProfileService::onWeb`) DMs a one-time 30-minute `web_login` link to the signup page, so a Telegram member reaches the web without typing their email. Removed `/stop` (and `OnboardingService::stop`) — the account-disable command — matching the web pause removal; per-type `/notifications` is the way to mute email now |
| v1.8.0 | ✅ Add-to-calendar `.ics` holds (`CalendarController`, signed `/calendar` route) in signup confirmations and the web profile; a "You're on next" upcoming-tasks block (`SignupBoardView::upcomingFor`); smoother passwordless sign-in (remembered email, resend, clearer copy); a one-tap "Cancel" button on the 24h Telegram reminder (reuses the `l:` leave callback → same drop + slot-freed path); the roster's per-row Update buttons replaced by one global "Update attendance" (`RosterPage::saveRoster`, HTML5 `form=` association). Removed the web pause-account control and the People "Open-task email" column; fixed the Settings roles table overflow |
| v1.7.8 | ✅ Moved the "Send open-task email now" action from the Settings page to the Tasks page (under the list) — `sendOpenTaskEmail` + the recipient count and nonce now live on `TasksPage`, and it redirects back to Tasks after sending |
| v1.7.7 | ✅ A role's default capacity can be set to 0 in Settings (`Roles::normalize` now clamps with `max(0, …)`, template `min="0"`), matching per-task capacity — the role stays defined but generates no active slots, so its tasks drop off the boards (`WHERE capacity > 0`) until raised |
| v1.7.6 | ✅ Group lock (Settings, on by default): `BoardService::moveBlocked()` keeps the board in the first group the bot joins — a second `my_chat_member` capture or a `/board` in another chat is ignored — so the board can't be hijacked into a different group. Untick to move it deliberately (going live) |
| v1.7.5 | ✅ The email account link is now a single-use, 30-minute `web_login` token (consumed on `template_redirect`, same magic-link the login form uses) rather than a reusable one, so a forwarded old email can't be replayed as a login. The hourly heartbeat sweeps spent and expired `auth_tokens` (`AuthTokenRepository::purgeExpired`) since one is now issued per email footer |
| v1.7.4 | ✅ The account link in every email now lands on the person's own signed-in profile page, where pausing email / deleting data lives — instead of a bare REST endpoint. Telegram-only installs without a public page keep the standalone self-service page |
| v1.7.3 | ✅ Account management on the signed-in web profile — pause all emails (switch the account off) or delete your data — reusing the signed `/manage` self-service page the emails link to. Packaged for the WordPress.org directory: `readme.txt` with external-service disclosure, completed plugin header, and `.wordpress-org/` icon + banner assets |
| v1.7.2 | ✅ The scheduled/manual open-task call now also reaches crew already working that day (same second-slot reasoning as v1.7.1); it no longer excludes anyone assigned that date. The send-once ledger still prevents duplicates |
| v1.7.1 | ✅ Fix: the freed-slot call after a late cancellation no longer skips people already working that day — they may want a second, non-overlapping slot; only the person who cancelled is left out |
| v1.7.0 | ✅ Per-type, per-channel notification preferences (schema 7→8, `notify_prefs`). New `NotificationPreferences` gate every notifier consults; a web **Notifications** tab and a `/notifications` Telegram keyboard menu set it (shared). Every notification now sends on both channels — closed the gaps (`/allow` grants + `/replace` covers now email too; the open-task call now DMs). Commitments (signup/cancel confirmation, reminder) and transactional mails are locked on; `notify_muted` retired (kept only as a legacy DM default seed). Extras: the door-ticket link is now in the signup DM; redeeming a credit at the door list DMs+emails the ticket; a late cancellation broadcasts a "spot just opened" call to the crew (`SlotFreedNotice`); new `/mytickets` command + a **My tickets** web list (upcoming + past) via a shared `TicketList` |
| v1.6.0 | ✅ Permissions + the crew-leader meta-role (schema 6→7). New `/allow` (leader / one-time at-risk pass / admin) and its People-page mirror; the pass waves an at-risk member through their next signup once. A reserved auto-managed 🧭 Leader slot appears as a board task (global default + per-event toggle on the Roster screen); its holder marks that night's crew from Telegram without being an organizer. Any task can now be set to 0 people, which drops it off both boards. Leader eligibility (≥ N completions in every active role) drives a new Leadership admin page, `/leaders`, and an hourly DM+email alert to organizers when someone qualifies. A `/gift` from Telegram now names the granting organizer in the credit-grant audit, not "the bot" |
