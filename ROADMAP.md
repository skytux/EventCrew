...
| v1.7.4 | ✅ The account link in every email now lands on the person's own signed-in profile page (trading a reusable signed `manage` token for a session via `template_redirect`), where pausing email / deleting data lives — instead of a bare REST endpoint. Telegram-only installs without a public page keep the standalone self-service page |
